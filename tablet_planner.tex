...
% Julian日からISO週番号を求める簡易関数
\makeatletter
\def\pgfcalendarjuliantoweek#1#2#3{%
  % #1 = Julian 日, #2 = 年用カウンタ, #3 = 週番号用カウンタ
  \pgfcalendarjuliantodate{#1}{#2}{\tempmonth}{\tempday}%
  % 年初のJulian値
  \pgfcalendardatetojulian{\the#2-01-01}{\julianJanOne}%
  % 曜日（0=日曜,1=月曜,...）
  \pgfcalendarjuliantoweekday{\julianJanOne}{\dowJanOne}%
  % 週のオフセット（月曜始まりに調整）
  \offsetMonday=\dowJanOne
  \advance\offsetMonday by -1
  \ifnum\offsetMonday<0 \advance\offsetMonday by 7 \fi
  % 経過日数
  \tempday=#1
  \advance\tempday by -\julianJanOne
  % ISO週番号を求める（整数除算）
  \tempweeknum=\tempday
  \advance\tempweeknum by \offsetMonday
  \divide\tempweeknum by 7
  \advance\tempweeknum by 1
  % 代入
  #3=\tempweeknum
}
\makeatother
...
%%  \begin{tikzpicture}
%%  \fill[accent!30] (0,0) rectangle (254mm,32mm);
%%  \node[anchor=west,font=\fontsize{36}{40}\selectfont\bfseries] at (10mm,16mm) {Week \weeknum: \monthname{\tempmonth} \the\tempday{} - \monthname{\theendmonth} \theendday};
