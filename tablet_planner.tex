...
\foreach \w in {1,...,54} {
  ...
  \foreach \d in {0,1,2,3,4,5,6} {
    ...
    \node[font=\fontsize{12}{14}\selectfont] at (\xcenter,384) {
      \hyperlink{daily-\thecyear-\ifnum\cmonth<10 0\fi\thecmonth-\ifnum\cday<10 0\fi\thecday}{\thecmonth/\thecday}
    };
  }
...
\foreach \doy in {1,...,365} {
  ...
  \hypertarget{daily-\thecyear-\ifnum\cmonth<10 0\fi\thecmonth-\ifnum\cday<10 0\fi\thecday}{}
  \begin{tikzpicture}[remember picture,overlay]
  \node[anchor=north west] at (current page.north west) {
  \begin{minipage}[t][452mm][t]{254mm}
  ...
  \node[anchor=west,font=\fontsize{36}{40}\selectfont\bfseries] at (10mm,16mm) {
    \dayfulname{\dowmonday}, \monthname{\thecmonth} \thecday
  };
  \node[anchor=east,font=\fontsize{16}{20}\selectfont] at (244mm,16mm) {
    \hyperlink{monthly-\thecmonth}{Monthly} | \hyperlink{weekly-\thecweeknum}{Weekly}
  };
  \end{tikzpicture}
